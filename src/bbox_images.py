"""
BBox-image classifier (OpenAI Vision)

- Reads bbox images (e.g., {original_index}_bbox.png) from BBOX_IMAGES_DIR
- Calls OpenAI Vision and extracts solar_panels = Yes/No
- Writes/updates: data/working/image_classification_tests.csv
  Adds/updates columns: original_index, image_name, bbox (Yes/No), marker (kept if exists)
...
# Where your bbox images were saved by your downloader script
BBOX_IMAGES_DIR = "data/images/test/bbox"

OUTPUT_CSV = "data/working/image_classification_tests.csv"

MAX_IMAGES = 5  # int for testing; None = no limit

REQUEST_DELAY = 2.0  # delay between images to avoid rate limits
MAX_RETRIES = 10
...
PROMPT_BBOX = """
Only consider the roof inside the red bounding box. Ignore other nearby roofs.
Baseline assumption: No solar panels unless you are extremely confident.

Task:
- Determine whether solar panels are installed on the selected roof.
...
def get_original_index_from_bbox_filename(p: Path) -> int | None:
    """
    Expect filenames like:
      10657_bbox.png
    but also tries to find any digits.
    """
    stem = p.stem  # e.g. "10657_bbox"
    m = re.match(r"^(\d+)_bbox$", stem)
    if m:
...
def upsert_results(output_csv: Path, rows: list[dict]) -> None:
    """
    rows: [{original_index:int, image_name:str, bbox:"Yes|No"}]
    ...
    if "bbox_new" in merged.columns:
        merged["bbox"] = merged["bbox_new"].combine_first(merged["bbox"])
        merged.drop(columns=["bbox_new"], inplace=True)
...
def main() -> None:
    ...
    images_dir = project_root / BBOX_IMAGES_DIR
    output_csv = project_root / OUTPUT_CSV
    output_csv.parent.mkdir(parents=True, exist_ok=True)

    if not images_dir.exists():
        raise SystemExit(f"BBox images dir not found: {images_dir}")
    ...
    for p in paths:
        idx = get_original_index_from_bbox_filename(p)
        if idx is not None:
            work.append((idx, p))

    if not work:
        print("No bbox images found to process.")
        return
    ...
    for i, (idx, path) in enumerate(work, 1):
        ...
        try:
            ...
            rows_out.append({"original_index": idx, "image_name": path.name, "bbox": solar})
            print(f"bbox={solar} (c={result['solar_confidence']:.2f})")
        except Exception as e:
            rows_out.append({"original_index": idx, "image_name": path.name, "bbox": pd.NA})
            print(f"ERROR: {e}")
    ...
    print(f"Done. Wrote/updated {output_csv} (updated bbox for {len(rows_out)} rows).")
